...
export function StoreActionsManager({ onClose }: { onClose: () => void }) {
  ...
  const add = useMutation({
    mutationFn: () => saveLeaderAction({
      store_id: storeId, title: title.trim(), due_label: due.trim() || undefined, assignee: assignee.trim() || undefined,
      repeat, repeat_dow: repeat === "weekly" ? repeatDow : undefined,
    }),
    onSuccess: () => { setTitle(""); setDue(""); setAssignee(""); setRepeat("none"); invalidate(); },
    onError: err,
  });
      ...
      <div className="max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-2xl border border-border bg-surface p-5 shadow-xl" onClick={(e) => e.stopPropagation()}>
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-1.5">
                    ...
                    {a.repeat && a.repeat !== "none" && (
                      <span className="rounded-full bg-blue-50 px-1.5 py-0.5 text-[10px] font-bold uppercase tracking-wide text-blue-700 ring-1 ring-inset ring-blue-200">
        ...
        <div className="mt-4 rounded-xl border border-border bg-surface-muted p-3">
          ...
          <div className="mt-1.5 flex items-center gap-1.5">
            <select value={repeat} onChange={(e) => setRepeat(e.target.value as "none" | "daily" | "weekly")} className={FIELD}>
              <option value="none">One time</option>
              <option value="daily">Repeats every day</option>
              <option value="weekly">Repeats weekly</option>
            </select>
            {repeat === "weekly" && (
              <select value={repeatDow} onChange={(e) => setRepeatDow(parseInt(e.target.value, 10))} className={FIELD}>
