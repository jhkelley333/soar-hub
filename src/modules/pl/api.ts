...
import type { ParsedPlStore, PlBonusCheck, PlCompare, PlComparePeriod, PlLine, PlOverviewRow, PlPeriod, PlStage, PlStatement } from "./types";
...
export function fetchPlBonusCheck(period: string): Promise<PlBonusCheck> {
  return request(`${FN}?action=bonus-check&period=${encodeURIComponent(period)}`);
}
