...
export function BudgetTargetsModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const enabledOf = (t: PlBudgetTarget) =>
    enabledEdits[t.line_key] !== undefined ? enabledEdits[t.line_key] : t.enabled;

  const changed = useMemo(() => {
    const keys = new Set([...Object.keys(edits), ...Object.keys(enabledEdits)]);
    const out: PlBudgetTarget[] = [];
    for (const t of targets) {
      if (!keys.has(t.line_key)) continue;
      const pctChanged = edits[t.line_key] !== undefined && edits[t.line_key] !== (t.target_pct == null ? "" : String(t.target_pct));
      const enChanged = enabledEdits[t.line_key] !== undefined && enabledEdits[t.line_key] !== t.enabled;
      if (pctChanged || enChanged) out.push(t);
    }
    return out;
  }, [edits, enabledEdits, targets]);
  ...
  const save = useMutation({
    mutationFn: () =>
      savePlBudget(
        changed.map((t) => {
          const v = valueOf(t);
          return { line_key: t.line_key, target_pct: v.trim() === "" ? null : Number(v), enabled: enabledOf(t) };
        }),
      ),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["pl-budget"] });
      qc.invalidateQueries({ queryKey: ["pl-review"] });
      qc.invalidateQueries({ queryKey: ["pl-review-summary"] });
      setEdits({}); setEnabledEdits({});
    ...
    <Modal open={open} onClose={onClose} title="Budget & Targets" maxWidth="max-w-lg">
      <div className="space-y-3">
        <p className="text-xs text-zinc-500">
          ...
          actuals against these. Uncheck a line to stop flagging it.{!canEdit && " (View only — Admin/COO/VP edit.)"}
        </p>
        {q.isLoading ? (
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-zinc-100">
                {targets.map((t) => {
                  ...
                  const off = !t.is_group && !on;
                  return (
                    <tr key={t.line_key} className={cn(t.is_group && "bg-amber-50/60", off && "opacity-50")}>
                      <td className="px-3 py-1.5 text-center">
                        {!t.is_group && (
                          <input
                            ...
                            checked={on}
                            disabled={!canEdit}
                            onChange={(e) => setEnabledEdits((p) => ({ ...p, [t.line_key]: e.target.checked }))}
                            className="h-4 w-4 rounded border-zinc-300 text-accent focus:ring-accent disabled:opacity-60"
                            title={on ? "Flagged in the review — click to turn off" : "Not flagged — click to turn on"}
                          />
                        )}
                      </td>
        ...
        {canEdit && (
          <div className="flex items-center justify-end gap-2">
            ...
            <Button variant="primary" size="sm" disabled={!changed.length || save.isPending} onClick={() => save.mutate()}>
