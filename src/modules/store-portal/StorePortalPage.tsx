...
import {
  AlertTriangle, ArrowRight, Check, Megaphone, Phone, PhoneCall, X,
} from "lucide-react";
        ...
        {contacts.map((c) => (
          <li key={c.slot} className="flex items-center gap-3 rounded-xl border border-zinc-200 px-4 py-3">
            ...
            <div className="min-w-0 flex-1">
              ...
              {!c.phone && <div className="text-xs text-zinc-400">No phone on file</div>}
            </div>
            {c.phone && (
              <a href={`tel:${c.phone}`} className="inline-flex items-center gap-1.5 rounded-lg bg-red-600 px-4 py-2 text-sm font-bold text-white transition hover:bg-red-700">
