...
function bandInput(r, p, ix, fcTarget) {
  ...
  return {
    ...
    // SALES + LY columns stay all-store (true revenue, matches the feed's
    // netSales/previousYearNetSales and the points/financial columns).
    lySales: numOrNull(r[p + "prev_year_net_sales"]),
    // vs-LY % uses Skunkworks' same-store COMPARABLE bases (0277) so the % matches
    // the feed's yoYNetSalesPercentage exactly; falls back to all-store if the
    // comp columns aren't captured yet.
    compSales: numOrNull(r[p + "net_sales_comp"]) ?? numOrNull(r[p + "net_sales"]),
    compLySales: numOrNull(r[p + "prev_year_net_sales_comp"]) ?? numOrNull(r[p + "prev_year_net_sales"]),
...
export async function runRankingNow(supa, user, opts = {}) {
  const issues = [];

  // 1. Anchor. Normally the most recent COMPLETED fiscal week's Sunday with
  // data; opts.weekEnding re-runs a SPECIFIC past week instead (the Refresh
  ...
  if (opts.weekEnding && /^\d{4}-\d{2}-\d{2}$/.test(opts.weekEnding)) {
    ...
    weekEnding = fiLatest.isWeekEnd ? latest : isoAddDays(fiLatest.weekStart, -1);
...
export async function latestRun(supa, params, storeNums = null) {
  ...
  if (params.run_id) {
    ...
    // "Latest" = the newest WEEK's newest run — ordered by week_ending first so
    // that re-running an OLDER week (Refresh) never hijacks the default view.
